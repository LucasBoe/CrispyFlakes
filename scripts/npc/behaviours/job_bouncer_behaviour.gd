...
func _find_respondable_fight(worker: NPCWorker) -> Fight:
	for fight: Fight in FightHandler.active_fights:
		if FightHandler.can_worker_respond_to_fight(worker, fight):
			return fight
	return null
