...
static var GUEST_SPAWN_SATISFACTION_EFFECT_STRENGTH = 0.9 # (base rate + count effect) * ((1.0 - 0.5) + (x * 0.5) - x = 0 > no effect x > full multiplication (e.g. 25% satisfaction > only 25% of guests spawn)
